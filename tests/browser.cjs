    ...
    try {
        ...
        const state = async (view, group) => {
            ...
            check(await page.locator('[data-ishi-primary]').evaluateAll(controls => controls.every(n => {
                ...
                return outline === !selected && solid === selected && style.backgroundColor === 'rgba(0, 0, 0, 0)' && style.color === ((selected || n.matches(':hover')) ? 'rgb(241, 139, 150)' : 'rgb(36, 37, 48)');
            })), 'Active icon is filled, with requested state color and transparent background');
        };
        check(await page.locator('.ishi-dashboard-primary').evaluate(n => getComputedStyle(n).borderBottomWidth === '0px'), 'Primary wrapper has no bottom border');
        check(await leaf('appointments').evaluate(n => getComputedStyle(n).backgroundColor === 'rgba(241, 139, 150, 0.19)' && getComputedStyle(n).color === 'rgb(241, 139, 150)'), 'Selected submenu colors');
        check(await leaf('history').evaluate(n => getComputedStyle(n).backgroundColor === 'rgba(119, 123, 129, 0.125)' && getComputedStyle(n).color === 'rgb(119, 123, 129)'), 'Normal submenu colors');
        check(await primary('account').evaluate(n => { const s=getComputedStyle(n); return s.color === 'rgb(36, 37, 48)' && s.borderBottomColor === 'rgba(0, 0, 0, 0.03)' && s.boxShadow.includes('rgb(221, 221, 221)'); }), 'Normal icon color border and shadow');
        ...
        for (const width of [320, 390, 768, 1280]) {
            ...
            for (const group of ['appointments', 'press-ons', 'messages', 'account']) {
                ...
                check(await primary(group).evaluate(n => { const s = getComputedStyle(n); return s.outlineStyle === 'none' && s.borderBottomColor === 'rgb(241, 139, 150)'; }), 'Pointer-selected primary has no focus outline and has requested underline');
            }
            await primary('appointments').click();
