...
check($x->query('//section[@data-ishi-section="press-ons"]//*[@data-ishi-secondary]')->length === 0, 'Press-Ons has no artificial secondary navigation');
...
check($x->query('//section[@data-ishi-section="appointments"]//a[@data-ishi-view="book"]')->length === 1, 'New Appointment remains an action');
...
check($x->query('//a[@data-ishi-primary="messages" and contains(@class,"latepoint-trigger-messages-tab")]')->length === 1, 'Primary Messages retains Pro click hook');
check($x->query('//a[@data-ishi-primary="messages"]//span[@class="lp-new-messages-count"]')->item(0)->textContent === '3', 'Unread badge retained');
foreach (array('calendar', 'nail', 'comment-light', 'avatar') as $icon) {
    check($x->query('//i[@aria-hidden="true" and contains(@class,"nails_skin_elementor_icons-' . $icon . '")]')->length === 1, 'Existing icon font class used');
}
...
check(xpath_for(render_dashboard($no_book))->query('//a[@data-ishi-view="book"]')->length === 0, 'Native hide booking setting respected');
...
check(strpos(render_dashboard($translated), '>History</a>') !== false, 'History label is independent of original language');
